...
class nnUNetDealiasLitModule(LightningModule):
    """Lightning module for regression to perform dealiasing.

    nnUNetDealiasLitModule is similar to nnUNetLitModule except for the choice of loss and
    evaluation metrics.
    """
    ...
    def __init__(
        ...
        self.example_input_array = MetaTensor(
            torch.rand(1, 2, *self.patch_size, device=self.device)
        )
