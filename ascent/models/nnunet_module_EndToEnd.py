...
class nnUNetLitModule(LightningModule):
    ...
    def __init__(
            self,
            net: torch.nn.Module,
            net_4cl: torch.nn.Module,
            net_5cl: torch.nn.Module,
        ...
        self.save_hyperparameters(logger=False, ignore=["net", "net_4cl", "net_5cl", "loss"])

        self.net = net
        self.net_4cl = net_4cl
        self.net_5cl = net_5cl
    ...
    def setup(self, stage: Optional[str] = None) -> None:  # noqa: D102
        ...
        self.num_classes = self.net_5cl.num_classes
    ...
    def forward(self, img: Union[Tensor, MetaTensor]) -> Union[Tensor, MetaTensor]:
        pred_3cl = self.net(img)
        img_secondStage = torch.cat((img, pred_3cl), 1)
        pred_4cl = self.net_4cl(img_secondStage)
        img_lastStage = torch.cat((img, pred_4cl), 1)
        # pred_5cl = self.net_5cl(img_lastStage)
        pred_5cl = torch.cat((pred_4cl, torch.zeros_like(pred_4cl[:, :1])), dim=1)
        return pred_3cl, pred_4cl, pred_5cl

    def model(self, img: Union[Tensor, MetaTensor]) -> Union[Tensor, MetaTensor]:  # noqa: D102
        pred_3cl = self.net(img)
        img_secondStage = torch.cat((img, pred_3cl), 1)
        pred_4cl = self.net_4cl(img_secondStage)
        # img_lastStage = torch.cat((img, pred_4cl), 1)
    ...
    def model_without_last(self, img: Union[Tensor, MetaTensor]) -> Union[Tensor, MetaTensor]:  # noqa: D102
        pred_3cl = self.net(img)
        img_secondStage = torch.cat((img, pred_3cl), 1)
        pred_4cl = self.net_4cl(img_secondStage)
    ...
    def training_step(
        ...
        # images_name = image_meta_dict["case_identifier"]

        # do_last_stage = torch.tensor([1 if s.endswith("D8") else 0 for s in images_name]).to(self.device)

        label_firstStage = torch.where((label == 3) | (label == 4), torch.tensor(2), label)
        label_secondStage = torch.where(label == 4, torch.tensor(3), label)

        # Need to handle carefully the multi-scale outputs from deep supervision heads
        pred_3cl = self.net(img)
        loss_3cl = self.compute_loss(pred_3cl, label_firstStage)

        img_secondStage = torch.cat((img, pred_3cl[0]), 1)
        pred_4cl = self.net_4cl(img_secondStage)
        loss_4cl = self.compute_loss(pred_4cl, label_secondStage)

        # img_lastStage = torch.cat((img, pred_4cl[0]), 1)
        # pred = self.net_5cl(img_lastStage)
        # loss_5cl = self.compute_loss(pred, label, do_last_stage)
        # pred = torch.cat((pred_4cl, torch.zeros_like(pred_4cl[:, :1])), dim=1)

        loss = loss_3cl + loss_4cl # + loss_5cl
    ...
    def validation_step(
        ...
        pred_3cl, pred_4cl, pred = self.forward(img)

        loss_3cl = torch.mean(self.loss(pred_3cl, label_firstStage))
        loss_4cl = torch.mean(self.loss(pred_4cl, label_secondStage))
        # loss_5cl = torch.mean(do_last_stage * self.loss(pred, label))
    ...
    def test_step(
        ...
        if properties_dict.get("resampling_flag"):
            spacing = properties_dict.get("spacing_after_resampling")

        start_time = time.time()
        preds = self.tta_predict(img, do_last_stage) if self.hparams.tta else self.predict(img, do_last_stage)
    ...
    def compute_loss(
            self, preds: Union[Tensor, MetaTensor], label: Union[Tensor, MetaTensor], do_last_stage: Union[Tensor, MetaTensor] = None
    ) -> float:
        ...
        Args:
            ...
            do_last_stage: If not None, indicates whose preds should include the last stage

        Returns:
            Train loss.
        """
        if do_last_stage is not None:
            if self.net.deep_supervision:
                loss = torch.mean(do_last_stage * self.loss(preds[0], label))
                for i, pred in enumerate(preds[1:]):
                    downsampled_label = nn.functional.interpolate(label, pred.shape[2:])
                    loss += 0.5 ** (i + 1) * torch.mean(do_last_stage *self.loss(pred, downsampled_label))
                c_norm = 1 / (2 - 2 ** (-len(preds)))
                return c_norm * loss
            return torch.mean(do_last_stage * self.loss(preds, label))
        else:
            if self.net.deep_supervision:
                loss = torch.mean(self.loss(preds[0], label))
                for i, pred in enumerate(preds[1:]):
                    downsampled_label = nn.functional.interpolate(label, pred.shape[2:])
                    loss += 0.5 ** (i + 1) * torch.mean(self.loss(pred, downsampled_label))
                c_norm = 1 / (2 - 2 ** (-len(preds)))
                return c_norm * loss
            return torch.mean(self.loss(preds, label))

    def predict(
            self, image: Union[Tensor, MetaTensor], do_last_stage: bool, apply_softmax: bool = True
    ) -> Union[Tensor, MetaTensor]:
        """Predict 2D/3D images with sliding window inference.

        Args:
            image: Image to predict.
            do_last_stage: Whether the last stage applies to this specific prediction.
        ...
        if len(image.shape) == 5:
            if len(self.patch_size) == 3:
                ...
                pred = self.predict_3D_3Dconv_tiled(image, do_last_stage, apply_softmax)
                # Inverse the padding after prediction
                return pred[..., (6 + extra_pad): (-6 - extra_pad)]
            elif len(self.patch_size) == 2:
                return self.predict_3D_2Dconv_tiled(image, do_last_stage, apply_softmax)
            else:
                raise NotImplementedError
        if len(image.shape) == 4:
            if len(self.patch_size) == 2:
                return self.predict_2D_2Dconv_tiled(image, do_last_stage, apply_softmax)
    ...
    def tta_predict(
        ...
        """
        preds = self.predict(image, do_last_stage, apply_softmax)
        for flip_idx in self.tta_flips:
            preds += torch.flip(self.predict(torch.flip(image, flip_idx), do_last_stage, apply_softmax), flip_idx)
    ...
    def predict_2D_2Dconv_tiled(
        ...
        if apply_softmax:
            return softmax_helper(self.sliding_window_inference(image, do_last_stage))
        else:
            return self.sliding_window_inference(image, do_last_stage)
    ...
    def predict_3D_2Dconv_tiled(
        ...
        for depth in range(image.shape[-1]):
            preds[..., depth] = self.predict_2D_2Dconv_tiled(image[..., depth], do_last_stage, apply_softmax)
        return preds
    ...
    def sliding_window_inference(
            self, image: Union[Tensor, MetaTensor], do_last_stage: bool
    ) -> Union[Tensor, MetaTensor]:
        ...
        if do_last_stage:
            ...
            return self.inferer(
                inputs=image,
                network=self.model_without_last,
            )
