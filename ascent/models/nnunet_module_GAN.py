...
from monai.transforms import KeepLargestConnectedComponent, Rotate, AsDiscrete
...
class nnUNetLitModule(LightningModule):
    ...
    def __init__(
        ...
        self.save_hyperparameters(logger=False, ignore=["net", "discriminator", "loss", "loss_d"])
        ...
        self.one_hot = AsDiscrete(to_onehot=self.net.decoder.num_classes, dim=1)
        ...
        self.test_step_hd = []
        self.test_step_hd_classes = []

        self.hd = HausdorffDistanceMetric(include_background=False, distance_metric='euclidean')
    ...
    def training_step(
        ...
        # Need to handle carefully the multiscale outputs from deep supervision heads
        ...
        for param in self.discriminator:
        ...
        logits_fake = self.discriminator(img_pred)
        adv_generator_loss = self.loss_d(logits_fake, target_is_real=True, for_discriminator=False)
        loss_g = loss + self.adv_weight * adv_generator_loss
        ...
        logits_fake = self.discriminator(img_pred.detach())
        loss_d_fake = self.loss_d(logits_fake, target_is_real=False, for_discriminator=True)
        ...
        loss_d_real = self.loss_d(logits_real, target_is_real=True, for_discriminator=True)
        ...
        self.log(
            "train/loss_gen_adv",
            self.adv_weight * adv_generator_loss,
            on_step=False,
            on_epoch=True,
            prog_bar=True,
            logger=True,
    ...
    def test_step(
        ...
        one_hot_pred = one_hot(pred_seg, num_classes).permute(0, 4, 1, 2, 3)
        one_hot_label = one_hot(label.to(torch.int64), num_classes).permute(0, 4, 1, 2, 3)

        test_hd = self.hd(one_hot_pred, one_hot_label, spacing=spacing)

        test_hd[test_hd == torch.inf] = torch.nan

        test_hd_classes = torch.nanmean(test_hd, 0)

        test_hd = torch.nanmean(test_hd, 1)
        test_hd = torch.nanmean(test_hd, 0)
        ...
        if self.hparams.save_predictions:
            ...
            if properties_dict.get("rotate_flag"):
                angle = properties_dict.get("angle_rotation")
                rotation = Rotate(angle=0-angle, mode="nearest")
                for slice in range(final_preds.shape[3]):
                    final_preds[:, :, :, slice] = rotation(final_preds[:, :, :, slice])
            ...
            self.test_step_hd.append({"test/hd": test_hd})
            self.online_test_tp.append({"test/tp_hard": tp_hard})
            self.online_test_fp.append({"test/fp_hard": fp_hard})
            self.online_test_fn.append({"test/fn_hard": fn_hard})
            self.test_step_hd_classes.append({"test/hd_classes": test_hd_classes})

        return {"test/dice": test_dice}

    def on_test_epoch_end(self):  # noqa: D102
        # Return mean hd and dice
        ...
        mean_hd = self.metric_mean("test/hd", self.test_step_hd)
        self.test_step_hd.clear()  # free memory
        ...
        # Return mean hd and dice per classes
        sum_tp = self.metric_sum("test/tp_hard", self.online_test_tp)
        sum_fp = self.metric_sum("test/fp_hard", self.online_test_fp)
        sum_fn = self.metric_sum("test/fn_hard", self.online_test_fn)

        self.online_test_tp.clear()  # free memory
        self.online_test_fp.clear()  # free memory
        self.online_test_fn.clear()  # free memory

        global_dc_per_class = [
            i if not torch.isnan(i) else 0.0
            for i in [
                2 * i / (2 * i + j + k)
                for i, j, k in zip(sum_tp, sum_fp, sum_fn)
            ]
        ]
        for label, dice in zip(range(len(global_dc_per_class)), global_dc_per_class):
            self.log(
        ...
        mean_hd_classes = self.metric_mean("test/hd_classes", self.test_step_hd_classes)
        for label, hd in zip(range(len(mean_hd_classes)), mean_hd_classes):
            self.log(
                f"test/hd/{label}",
                hd,
                on_step=False,
    ...
    def configure_optimizers(self) -> tuple[Union[dict[str, Any], dict[str, Any]], ...]:
        ...
        configured_optimizer_d = {
            "optimizer": self.hparams.optimizer_d(params=self.discriminator.parameters())
        }
    ...
    def metric_mean(name: str, outputs: list[dict[str, Tensor]]) -> Tensor:
        ...
        """
        return torch.nanmean(torch.stack([out[name] for out in outputs]), dim=0)

    @staticmethod
    def metric_sum(name: str, outputs: list[dict[str, Tensor]]) -> Tensor:
        """Average metrics across batch dimension at epoch end.
        ...
        Returns:
            Averaged metrics tensor.
        """
        return torch.stack([out[name] for out in outputs]).sum(dim=0)
